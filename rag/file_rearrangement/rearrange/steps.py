...
# =============================================================================
# 0. Pre-enrichment merge (team-1 → team-2 handoff)
# =============================================================================
...
def reorganize_tree_by_final_paths(tree: Dict, final_paths_doc: Dict) -> Dict:
    ...
    for entry in entries:
        ...
        if source_fd is not None:
            ...
            h = source_fd.get("file_hash") or src
            key = (h, final_path)
            if key in seen_keys:
                continue
            seen_keys.add(key)
    ...
    print(
        ...
        f"({placed_with_tree} backed by a tree node, "
        f"{placed_as_placeholder} placeholders for files team 1 classified "
        f"but the first-pass tree omitted)."
    )
    return new_root
...
def _annotate_folder_sequences(node: Dict) -> None:
    """Set ``by_sequence=True`` on folders whose direct file children share a
    single ``sequence_name``. Recurses through the rebuilt tree."""
    files = node.get("files") or {}
    if isinstance(files, dict) and files:
        seqs = {fd.get("sequence_name") for fd in files.values() if fd.get("sequence_name")}
        if len(seqs) == 1:

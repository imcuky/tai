...
def build_hw_orphans(hw_node: Dict, db_path: str) -> List[Dict]:
	orphans: List[Dict] = []
	...
	def recurse(folder_node: Dict, folder_path: str) -> None:
		...
		if file_nodes and not child_folders:
			part_names: List[str] = []
...
def main() -> None:
	parser = argparse.ArgumentParser(
		description="Test direct multi-match using only HW orphans against existing backbone groups"
	)
	parser.add_argument(
		"--input",
	...
	parser.add_argument(
		"--orphans-out",
		default=os.path.join(REARRANGE_DIR, "logs", "test_hw_orphans.json"),
		help="Where to write generated HW orphan payload.",
	)
	parser.add_argument(
		"--matches-out",
		default=os.path.join(REARRANGE_DIR, "outputs", "test_hw_orphan_matches.json"),
		help="Where to write match results.",
	)
	args = parser.parse_args()

	tree = load_json_file(args.input)
	hw_node = find_child_folder(tree, "hw")
	if not hw_node:
		raise ValueError("Could not find 'hw' folder in input tree.")

	groups_data = load_json_file(args.groups)
	groups = [BackboneGroup(**g) for g in groups_data]

	orphans = build_hw_orphans(hw_node, args.db)
	save_json(orphans, args.orphans_out)
	print(f"Prepared {len(orphans)} HW orphan items -> {args.orphans_out}")
